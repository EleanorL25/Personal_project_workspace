...
Combined_cor<-cor.test(combined_Temp_tree_data$Temp_average, combined_Temp_tree_data$Tree_loss_ha, method = "pearson")
...
```{r}
combined_income_temp<-cross_join(combined_Temp_tree_data, median_income_data)

Cor_income_temp<-cor.test(combined_income_temp$Temp_average, combined_income_temp$med_hh_income, method = "pearson")

print(Cor_income_temp)

```

Based on the basic correlation test computed between median income in LA vs average yearly temperatures, there is exceedingly high correlation. Further testing is required to confirm this result as the p-value was 1. This is incredibly uncommon for different variables and leads us to believe there is more to analyze before drawing conclusions.
